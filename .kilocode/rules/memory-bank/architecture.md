...
-   **Dynamic Intent Recognition:** Processes user queries through a database-driven intent recognition system that differentiates between general feature mentions (e.g., "wc") and specific intent queries (e.g., "área do wc"). Intent instructions are dynamically injected into LLM prompts based on client-specific configurations stored in the `clients` table.
-   **Dynamic Prompt Construction:** Creates detailed prompts with client-specific instructions, incorporating aggregated structured data from Supabase and dynamically matched intent instructions.

...
const { originalname, buffer } = file;
  ...
  try {
    ...
    if (documentCategory === 'listing') {
      ...
      console.log(`[DEBUG] Document category is 'listing'. currentListingId: ${currentListingId}, currentDevelopmentId: ${currentDevelopmentId}`);
      ...
      console.log(`[DEBUG] listingData before Supabase call:`, JSON.stringify(listingData, null, 2));

      if (currentListingId) {
        // Update existing listing
        try {
          console.log(`[DEBUG] Attempting to update listing with ID: ${currentListingId}`);
          const updatedListing = await listingService.updateListing(currentListingId, listingData);
          if (updatedListing) {
            console.log(`[${clientConfig.clientName || clientConfig.clientId}] Listing ${currentListingId} updated in Supabase. Result:`, JSON.stringify(updatedListing, null, 2));
          } else {
            console.log(`[DEBUG] Listing ${currentListingId} not found for update. Attempting to create.`);
            // If update didn't find a record, it means it doesn't exist, so create it
            listingData.id = currentListingId; // Use the provided listing_id as the ID for creation
            const createdListing = await listingService.createListing(listingData);
            currentListingId = createdListing.id; // Ensure currentListingId is updated
            console.log(`[${clientConfig.clientName || clientConfig.clientId}] New listing ${currentListingId} created in Supabase (after failed update). Result:`, JSON.stringify(createdListing, null, 2));
          }
        } catch (error) {
          console.error(`[${clientConfig.clientName || clientConfig.clientId}] Error updating or creating listing ${currentListingId} in Supabase:`, error);
          throw new Error(`Failed to update or create listing: ${error.message}`);
        }
      } else {
        // Create new listing if no currentListingId was provided
        listingData.id = uuidv4(); // Generate UUID for new listing
        try {
          console.log(`[DEBUG] Attempting to create new listing with generated ID: ${listingData.id}`);
          const createdListing = await listingService.createListing(listingData);
          if (createdListing) {
            currentListingId = createdListing.id;
            console.log(`[${clientConfig.clientName || clientConfig.clientId}] New listing ${currentListingId} created in Supabase. Result:`, JSON.stringify(createdListing, null, 2));
          } else {
      ...
      // Ensure currentListingId is set from the successfully created/updated record
      if (!currentListingId) {
        throw new Error(`Failed to obtain a valid listing ID after processing.`);
      }

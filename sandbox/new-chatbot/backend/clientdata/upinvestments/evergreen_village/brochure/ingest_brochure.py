#!/usr/bin/env python3
"""
Brochure → Pinecone RAG ingestion

Usage:
  py ingest_brochure.py brochure.pdf --index new-chatbot-1536 --namespace evergreen_village_marketing \
     --dataset evergreen_village_marketing --version v1 --language pt --query "apartamento T2 com varanda"
...
Notes:
  ...
  - Works with Pinecone client v2.x (preferred) and falls back to legacy init if needed
"""
...
def connect_pinecone(index_name: str):
    ...
    # Detect installed API surface (looks like an older 1.x client with functions)
...
def read_pdf_pages(pdf_path: str) -> List[str]:
    ...
    with pdfplumber.open(pdf_path) as pdf:
        for page in pdf.pages:
            ...
            txt = txt.strip()
            # collapse single newlines within paragraphs, preserve blank lines as paragraph breaks
...
def upsert_with_backoff(index, vectors: List[dict], namespace: str, retries: int = 5, backoff: float = 1.5):
    ...
    while True:
        try:
            # prefer kwargs; fallback to legacy signatures
            try:
...
def main(argv: List[str]) -> int:
    ap = argparse.ArgumentParser(description="Ingest a brochure PDF into Pinecone for RAG")
    ap.add_argument("pdf", help="Path to PDF file")
    ap.add_argument("--index", default="new-chatbot-1536")
    ap.add_argument("--namespace", default="evergreen_village_marketing")
    ap.add_argument("--dataset", default="evergreen_village_marketing")

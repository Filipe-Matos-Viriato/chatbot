#!/usr/bin/env python3
"""
Evergreen Pure Units → Pinecone RAG ingestion

Usage:
  py ingest_units.py --dir . \
    --index new-chatbot-1536 \
    --namespace evergreen_pure_units \
    --dataset evergreen_pure_units \
    --version v1 \
    --language pt \
...
def ingest_file(index, oai: OpenAI, pdf_path: str, *, namespace: str, dataset: str, version: str,
    ...
    for c, vec in zip(all_chunks, embeddings):
        ...
        # Best effort: add typology/block extracted from filename convention
        try:
            name = os.path.basename(pdf_path)
            if name.lower().startswith("apartamento "):
                metadata["unit_label"] = name
        except Exception:
            pass
...
def main(argv: List[str]) -> int:
    ap = argparse.ArgumentParser(description="Ingest Evergreen Pure unit PDFs into Pinecone")
    ap.add_argument("--dir", default=".", help="Root directory to scan for PDFs")
    ap.add_argument("--index", default="new-chatbot-1536")
    ap.add_argument("--namespace", default="evergreen_pure_units")
    ap.add_argument("--dataset", default="evergreen_pure_units")
